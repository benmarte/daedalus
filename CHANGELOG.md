## [fix: retry-cap notification re-sent on every dispatch tick — ignores dedup markers and stage recovery](https://github.com/benmarte/daedalus/issues/1167) — [PR #1172](https://github.com/benmarte/daedalus/pull/1172)

Retry-cap notifications were re-sent on every 5-minute dispatch tick because the dedup marker was not role-scoped (a new card id per retry meant the marker never matched) and the notification logic did not verify the stage was still stalled. The fix introduces role-scoped markers (`<!-- daedalus:retry-cap-notified:<role> -->`), a `_retry_cap_stage_recovered()` check that suppresses notifications when a newer card is running/done, an open PR exists, or a downstream role is active, and a fallback stamp target so the marker always persists on the triggering card when the validator card is not found. GitHub cap-exhaustion comments are also suppressed on recovery.
