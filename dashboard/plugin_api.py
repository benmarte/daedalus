...
# ``projects_router`` and ``project_config_router`` are now defined in
# ``dashboard/routes/projects.py`` and ``dashboard/routes/project_config.py``
# respectively (issue #1155, PR 2/3).  They are imported below after all helper
# functions are defined (to avoid circular imports) and re-exported here so
# historical ``dashboard.plugin_api.projects_router`` references keep resolving.
...
# ── Tracking mode, project-entry builders, and per-project config endpoints ──
...
# (issue #1155, PR 2/3).  The sub-routers are imported and included at the
# *bottom* of this module (after all helper functions) so route handlers can call
# patchable names (_fetch_project_tasks, _kanban_summary, _needs_attention,
# _project_provider, _open_prs, _cron_health_all, _hermes_cmd, ensure_board,
# _reconcile_cron) through this module's namespace and all existing
# ``mock.patch("dashboard.plugin_api.<name>")`` test targets remain intact.
...
# router objects so ``dashboard.plugin_api.projects_router`` and
# ``dashboard.plugin_api.project_config_router`` keep resolving (tests use them).
