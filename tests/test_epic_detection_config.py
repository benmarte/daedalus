...
class TestResolveEpicConfig:
    ...
    def test_enabled_coerced_from_string(self):
        assert (
            disp._resolve_epic_config({"epic_detection": {"enabled": "false"}})[
                "enabled"
            ]
            is False
        )
        ...
        assert (
            disp._resolve_epic_config({"epic_detection": {"enabled": "yes"}})["enabled"]
            is True
        )
        assert (
            disp._resolve_epic_config({"epic_detection": {"enabled": "1"}})["enabled"]
            is True
        )
        assert (
            disp._resolve_epic_config({"epic_detection": {"enabled": "on"}})["enabled"]
            is True
        )
        assert (
            disp._resolve_epic_config({"epic_detection": {"enabled": "0"}})["enabled"]
            is False
        )
        assert (
            disp._resolve_epic_config({"epic_detection": {"enabled": "no"}})["enabled"]
            is False
        )
...
class TestValidationWarnings:
    ...
    def test_min_deliverables_negative_falls_back(self, caplog):
        with caplog.at_level(logging.WARNING):
            out = disp._resolve_epic_config(
                {"epic_detection": {"min_deliverables": -5}}
            )
        assert out["min_deliverables"] == 6
    ...
    def test_min_deliverables_non_int_falls_back(self, caplog):
        with caplog.at_level(logging.WARNING):
            out = disp._resolve_epic_config(
                {"epic_detection": {"min_deliverables": "not-a-number"}}
            )
        assert out["min_deliverables"] == 6
    ...
    def test_size_threshold_non_int_falls_back(self, caplog):
        with caplog.at_level(logging.WARNING):
            out = disp._resolve_epic_config(
                {"epic_detection": {"size_threshold": None}}
            )
        assert out["size_threshold"] == 1000
...
class TestIsEpicBackwardCompat:
    ...
    def test_2000_char_body_alone_not_epic(self):
        # Body size alone is no longer sufficient (#1100): needs decomp signal.
        assert is_epic(_make_issue(body="x" * 2000)) is False

    def test_2000_char_body_with_decomp_language_is_epic(self):
        body = "x" * 2000 + " decompose into phases"
        assert is_epic(_make_issue(body=body)) is True
...
class TestIsEpicConfigOverrides:
    ...
    def test_config_raises_body_size_threshold(self):
        # 1500 chars + decomp language: not epic under module threshold (2000),
        # but epic under config threshold=1200.
        cfg = {
            "enabled": True,
            "min_deliverables": 6,
            "size_threshold": 1200,
        ...
        body = "x" * 1500 + " decompose into phases"
        assert is_epic(_make_issue(body=body), epic_config=cfg) is True
    ...
    def test_custom_child_label_excludes(self):
        ...
        # 'subtask' label no longer guards; add decomp language to satisfy body-size
        # semantic check (threshold=100 so 5000 chars qualifies, needs decomp signal).
        assert (
            is_epic(
                _make_issue(
                    body="x" * 5000 + " decompose into phases",
                    labels=[{"name": "subtask"}],
                ),
                epic_config=cfg,
            )
            is True
        )  # old guard label no longer effective
...
class TestPlannerBodyReadsConfig:
    ...
    def test_config_custom_checklist_threshold(self):
        ...
        body = disp._planner_body("o/r", issue, "/w", "main", "github", epic_config=cfg)
        assert "checklist" in body.lower()
