...
class TestIsEpicBackwardCompat:
    ...
    def test_2000_char_body_not_epic_without_semantic_signals(self):
        # Body size alone no longer triggers epic (#1100)
...
class TestIsEpicConfigOverrides:
    ...
    def test_config_body_size_threshold_not_standalone_trigger(self):
        # Body size alone is not a trigger even with config size_threshold (#1100)
        cfg = {
            "enabled": True,
            "min_deliverables": 6,
            "size_threshold": 1200,
    ...
    def test_custom_child_label_excludes(self):
        ...
        # custom child_label excludes when matched
        assert (
            is_epic(
        ...
        # 'subtask' no longer guards when custom child_label is set;
        # use epic label (not body size) to verify the guard is bypassed (#1100)
        assert (
            is_epic(
                _make_issue(labels=[{"name": "subtask"}, {"name": "epic"}]),
                epic_config=cfg,
            )
            is True
        )  # old guard label no longer effective; epic label triggers
