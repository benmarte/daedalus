...
- Click **+ Add notification target** to add a channel. Each target can be filtered to specific event types (`doc-report`, `dispatch-summary`, `pipeline-failure`, `pr-ready`, `security-escalation`, `retry-cap-exhausted`, `retry-attempt`).
- **`retry-attempt`** fires on every intermediate retry (before the cap is hit), so you can watch a flaky agent approach its limit. Routed through `hermes send` only.
- **`retry-cap-exhausted`** fires when a PM or validator agent exhausts its retry cap without reaching CONFIRMED status, indicating manual intervention is required. Routes through both `hermes send` (to `cron.notifications` targets) **and** an independent outbound webhook channel — set `SLACK_WEBHOOK_URL` and/or `DISCORD_WEBHOOK_URL` in the dispatcher environment to deliver a structured critical-severity payload even when no `cron.notifications` targets are configured. The webhook POST is non-blocking (daemon thread) and failures are logged but never raise.
...
**Retry-cap exhaustion notifications:** validator and PM retry caps exist so a broken issue doesn't loop forever. But a cap with no notification means the operator only learns about it days later by scrolling the board. When a validator or PM agent exhausts its retry cap without reaching CONFIRMED status, the dispatcher posts a one-time `retry-cap-exhausted` notification (deduped per issue via a marker comment `<!-- daedalus:retry-cap-notified -->`) and routes it to the same channels as `security-escalation`. Intermediate retries fire a distinct `retry-attempt` event before the cap is hit, so operators can watch a flaky agent approach its limit in real time. Both events are routed through `hermes send` to any configured `cron.notifications` targets subscribed to them.

In addition, `retry-cap-exhausted` fires an **independent outbound webhook** — set `SLACK_WEBHOOK_URL` and/or `DISCORD_WEBHOOK_URL` in the dispatcher environment and the dispatcher POSTs a structured critical-severity payload (Slack Block Kit / Discord embed, context fields: `issue`, `role`, `retry_count`, `max_retries`, `recovery`) directly to each URL in a daemon thread, regardless of whether any `cron.notifications` targets exist. Webhook delivery is best-effort: failures are logged but never raise. This surfaces the wedge the moment it happens so a human can investigate. Configure both events in your notification targets to receive these alerts.
