...
def _install_cron_wrapper() -> tuple[bool, str]:
    ...
    script_content = (
        ...
        "# ---------------------------------------------------------------------------\n"
        "\n"
        "# --- Gateway watchdog (#187) ------------------------------------------------\n"
        "# The dispatcher only nudges the kanban board; the Hermes gateway is what\n"
        "# actually spawns agents. If the gateway has died silently the dispatcher\n"
        "# still exits 0 and cron looks healthy while no agents ever spawn. Detect a\n"
        "# dead gateway and attempt a single restart before dispatching.\n"
        "# NOTE: `hermes gateway status` always exits 0, so liveness MUST be read from\n"
        "# stdout (the 'not running' marker), never from the exit code. Best-effort:\n"
        "# a missing CLI or a failed restart only logs to stderr and never blocks the\n"
        "# dispatch below.\n"
        "if command -v hermes >/dev/null 2>&1; then\n"
        "  if hermes gateway status 2>&1 | grep -qi 'not running'; then\n"
        "    echo \"daedalus-cron: gateway appears DOWN — attempting restart\" >&2\n"
        "    hermes gateway restart >/dev/null 2>&1 || \\\n"
        "      echo \"daedalus-cron: gateway restart command failed\" >&2\n"
        "    # Give the gateway a moment to come back before the dispatcher nudges it.\n"
        "    sleep 5\n"
        "    if hermes gateway status 2>&1 | grep -qi 'not running'; then\n"
        "      echo \"daedalus-cron: gateway still DOWN after restart attempt\" >&2\n"
        "    else\n"
        "      echo \"daedalus-cron: gateway restart succeeded\" >&2\n"
        "    fi\n"
        "  fi\n"
        "fi\n"
        "# ---------------------------------------------------------------------------\n"
