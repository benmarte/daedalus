...
```
- Validator runs alone as Phase 1. Downstream tasks NOT created until validator emits `CONFIRMED:` (the default per-tick flow). The flag-gated alternative path (`pipeline.upfront_dag: true`, default off — see Config system) instead builds this whole stage graph upfront at Ready-time as a dependency-blocked DAG and prunes it by the validator verdict via a 6-outcome arbiter.
...
- **`protocol.prefix_fallback: false`** (flip after soak): JSON is required. A card with no valid JSON record routes to `PENDING_SIGNAL` (hold); `_guard_prefix_on_done` requires a valid record; marker reads skip comment-scan when `workdir` is threaded through (currently inert — no existing call site passes `workdir` to `_has_notified_block`/`_is_consult_resolved`; threading deferred as follow-up, so marker reads remain comment-scan-backed regardless).
- **Flip criteria:** `_outcome_json` ≥ 95% across ≥ 7 consecutive production days with no regressions. Fork 3 marker-read optimisation requires the separate `workdir`-threading follow-up; there is no state-warmup precondition for flipping the flag. See `templates/daedalus.yaml` `protocol:` section.
- **`protocol.metadata_transport: false`** (default — behaviour unchanged; #1288, Phase 1 of epic #1276): when `true`, **completion** handoffs additionally emit the structured outcome as native run metadata via `hermes kanban complete --metadata` (read back with `hermes kanban runs --json`). `classify_blocked()` reads the native closing-run outcome FIRST (telemetry source `metadata`), then falls through to the free-text JSON block, then the legacy prefix — no routing is lost; `_guard_prefix_on_done` also accepts a metadata-only completion. **Blocked** handoffs (review-required, awaiting-pr, changes-requested, needs_input) still carry free-text JSON because `hermes kanban block` has no `--metadata` option (full elimination awaits #1290, Phase 2). With the flag off the caller passes `native_outcome=None`, so routing is byte-identical. Keep `prefix_fallback: true` while soaking.
- Full design in issue #1170.
- **`pipeline.upfront_dag: false`** (default — behaviour byte-identical; #1290, Phase 2 / KEYSTONE of epic #1276): when `false` the dispatcher creates ONLY the validator card at Ready-time and fans out the downstream stages per-tick after the validator emits `CONFIRMED:` (the classic phased flow). When `true`, the ENTIRE stage graph is built in one shot at Ready-time — validator → pm → developer → qa → [reviewer, security, accessibility] → docs — with every non-root stage created dependency-blocked (`hermes kanban block --kind dependency`) so Hermes auto-promotes it the moment ALL of its parents complete (docs is multi-parented to reviewer + security + accessibility). A 6-outcome arbiter then prunes the pre-built DAG by the validator's structured verdict (read from native run metadata first per #1288, then free-text-JSON, then legacy prefix): `CONFIRMED` keeps the DAG; `ALREADY_FIXED`/`DUPLICATE` cancel every downstream branch; `NEEDS_MORE_INFO`/`BLOCK_FOR_REVIEW` block for human input (`--kind needs_input`); `SECURITY_THREAT` escalates + cancels; anything unparseable safe-parks as `NEEDS_MORE_INFO` (never auto-proceeds). While the flag is on, the per-tick post-developer downstream creation no-ops (the upfront DAG owns those cards) and re-ticks are idempotent via stable `<role>-{n}` keys. Keep `protocol.metadata_transport: true` alongside so the arbiter can read the validator verdict natively. Highest-risk toggle — ships inert; **flipping it on is gated on the body-enrichment follow-up #1301** (intermediate stage cards get generic bodies until then). Downstream work (#1291 gates, #1292 labels, #1294 decompose) builds on this. Full design in issue #1290.
- **`pipeline.concurrent_dedup.enabled: false`** (default — behaviour byte-identical; #1413): concurrent-duplicate detection for same-root-cause issues processed in parallel (a case the validator's sequential open-PR/issue check structurally cannot catch — a concurrently-processing sibling has no PR yet at validation time). Pure logic in `core/concurrent_dedup.py` (built on `core/file_overlap.py`), wired into `_run_tick`. **Layer 1** (dispatch-time hold): before creating a validator card for issue X, `find_inflight_duplicate()` checks file/keyword overlap (≥ `min_confidence`, default 0.8) against in-flight siblings (`gather_inflight_siblings()` = managed ∪ created-this-tick, still in `issues_map`); on a hit X is held (no card created, board status untouched) and surfaced under `concurrent_holds` in the tick summary — the hold auto-releases when the sibling's issue closes and drops out of `issues_map`. **Layer 2** (pre-merge PR gate): when an open PR is reconciled, `find_pr_file_overlap()` compares its changed files against other open PRs and, if the *newer* PR shares ≥ `min_shared_files` (default 1, suffix-aware) with an older open PR, posts a one-shot `dup_warned` "suspected duplicate" comment for human confirmation — it never auto-closes a PR. Flag-off → both gates are inert (no extra provider calls). See `templates/daedalus.yaml` `pipeline.concurrent_dedup` and issue #1413.
- **`pipeline.native_gates: false`** (default — behaviour byte-identical; #1291, Phase 3 of epic #1276): when `true`, the in-pipeline human gate — the developer `review-required: PR #N` re-block emitted once the PR is open and awaiting human review/merge (QA/reviewer/security run in the meantime) — is tagged with the native Hermes block category `hermes kanban block --kind needs_input`, so an awaiting-human card is natively distinguishable on the board from a machine-wait. The `kind` is advisory metadata only: `classify_blocked()` still routes off the `review-required:` reason string (→ ADVANCE), so flag-on adds the tag without changing flow and flag-off omits `--kind` entirely (plain block, byte-identical). Machine-wait blocks (`awaiting-fix:`, `awaiting-pr`) are deliberately NOT tagged — they have no `--parent` edges and must not be mistaken for a human gate. The 3 GitHub-side gates (move-to-Ready, merge action, close-issue) stay human/assert-only — they are never card blocks. The native pending-merge (`auto_merge=false`) `needs_input` marker is a documented follow-up: the docs card is terminal by merge-time so there is no non-fragile card to block. See issue #1291.
