...
- **Do NOT create or modify `CHANGELOG.md`.** It is auto-generated by the CI workflow on the base branch when a PR merge commit lands. Editing it in a PR branch causes concurrent PRs to conflict on line 1 (#1179) — never add, stage, or commit `CHANGELOG.md`.
...
3. **Enumerate tracked docs.** List every markdown file in the repo **root** and in `docs/` (e.g. `README.md`, `SETUP.md`, `CONTRIBUTING.md`, `docs/INSTALLATION_GUIDE.md`, ADRs). Use `git ls-files '*.md' 'docs/*.md'` so it is project-agnostic — never assume a fixed list. **Exclude `CHANGELOG.md`** from this set: it is owned by the CI workflow and must never be edited in a PR branch (#1179).
